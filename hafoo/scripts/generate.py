...
# hafoo 目录：hafoo/
# ============================================================
HAFOO_DIR = os.path.dirname(SCRIPT_DIR)

# ============================================================
# 仓库根目录：my-rules/
# ============================================================
ROOT_DIR = os.path.dirname(HAFOO_DIR)

# ============================================================
# 数据目录：hafoo/data/
# ============================================================
DATA_DIR = os.path.join(HAFOO_DIR, "data")
...
def load_domains(region):
    """读取 hafoo/data/{region}-source.txt"""
...
def write_list(region, domains):
    """生成 hafoo_hk.list（Clash / Surge 通用）"""
    out = os.path.join(ROOT_DIR, f"hafoo_{region}.list")
...
def write_yaml(region, domains):
    """生成 hafoo_hk.yaml（你指定的格式）"""
    out = os.path.join(ROOT_DIR, f"hafoo_{region}.yaml")
    with open(out, "w") as f:
        f.write("payload:\n")
        for d in domains:
            if d.startswith("KEYWORD:"):
...
def write_srs(region, domains):
    """生成 hafoo_hk.srs（Surge DOMAIN-SUFFIX 格式）"""
    out = os.path.join(ROOT_DIR, f"hafoo_{region}.srs")
    with open(out, "w") as f:
        for d in domains:
            if d.startswith("KEYWORD:"):
...
def write_readme(all_stats):
    ...
    for region in REGIONS:
        file_list += f"- `hafoo_{region}.list`\n"
        file_list += f"- `hafoo_{region}.yaml`\n"
        file_list += f"- `hafoo_{region}.srs`\n"

    readme_path = os.path.join(HAFOO_DIR, "README.md")
